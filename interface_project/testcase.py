...
class TestCase:
    def __init__(self, testcase_id, testcase_name, steps, active_status, testproject):
        ...
        logger.info("testcaseid=%s"%self.testcase_id)
        self.testcase_name = testcase_name
        self.steps = steps
        self.active_status = active_status # 用例是否禁用 1 - 活动 0 - 禁用
        self.testproject = testproject
        logger.info("testproject=%s" % self.testproject)
    def run_testcase(self, http, testplan):
        ...
        #调试david
        #logger.info("打印steps信息==%s"%self.steps)

        #self.steps = other_tools.conver_date_from_testlink(self.steps)
        #self.steps = eval(self.steps)
        #logger.info("打印steps信息===========%s" % self.steps)
